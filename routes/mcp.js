...
function createAthena(config, accountId, profile) {
	...
	// Banner
	profile.items.sandbox_loadout.attributes.locker_slots_data.banner_icon_template =
		config.BannerIconTemplate;
	profile.items.sandbox_loadout.attributes.locker_slots_data.banner_color_template =
	...
	stats.lifetime_wins = 137000;
	stats.level = parseInt(config.level, 10);
	stats.battlestars = parseInt(config.battlestars);
	stats.battlestars_season_total = 1; // battle stars
	stats.alien_style_points = 0; // alien artifacts
	stats.accountLevel = 125;
